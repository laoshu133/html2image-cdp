/**
 * fork from: https://github.com/shutterstock/changeDPI/blob/master/src/index.js
 */
...
module.exports = function changeDpiBuffer(buf, dpi, type = PNG) {
    const headerChunk = buf.slice(0, 33);
    const dataArray = new Uint8Array(headerChunk);
    const changedArray = changeDpiOnArray(dataArray, dpi, buf.type || type);

    return Buffer.concat([changedArray, buf.slice(33)]);
};
